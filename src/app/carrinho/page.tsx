...
export default async function CartPage() {
  ...
  return (
    <AccountShell currentPath="/carrinho" title="Carrinho" description="Revise seus itens e finalize a compra via PIX no carrinho.">
      ...
      {cartItems.length === 0 ? (
        <div className="mt-6 rounded-[28px] border border-white/80 bg-white/90 p-6 shadow-sm text-center text-slate-600">
          Nao ha itens no carrinho. Selecione uma cota em algum bolao para adicionar ao carrinho.
        </div>
      ) : null}
    </AccountShell>
  );
