...
export function PixCopyActions({ qrCodeText, paymentLinkUrl }: PixCopyActionsProps) {
  ...
  if (!qrCodeText && !paymentLinkUrl) return null;
    ...
    <div className="space-y-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
        {qrCodeText ? (
          <button
            type="button"
            onClick={() => void handleCopy(qrCodeText, "Código PIX")}
        ...
        {paymentLinkUrl ? (
          <button
            type="button"
            onClick={() => void handleCopy(paymentLinkUrl, "Link")}
            className="inline-flex items-center justify-center gap-2 rounded-full border border-fuchsia-200 bg-white px-4 py-3 text-sm font-semibold text-fuchsia-700 transition hover:bg-fuchsia-50"
          >
            <Copy className="h-4 w-4" />
            Copiar link
